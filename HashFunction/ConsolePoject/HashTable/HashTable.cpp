...
void HashTable::changeHashFunction(HashFunction* newHashFunction) {
    ...
    std::cout<<"new vector with old nodes"<<std::endl;
    for(int i=0; i < allNodes.size(); i++){
        std::cout<< allNodes[i] << " (" << allNodes[i]->getKey() << " " << allNodes[i]->getValue() << ") "<<std::endl;
    ...
    for(int i=0; i < allNodes.size(); i++){
       ...
       add(allNodes[i]->getKey(), allNodes[i]->getValue());
